...
def _prompt_for(output_type: str, product: dict[str, Any], project: dict[str, Any] | None = None) -> str:
    ...
    prompts = {
        ...
        "certificate": (
            ...
            + "Create one realistic phone-style angled top-down snapshot of the uploaded product with a readable physical product certificate. "
        ...
        "package": (
            ...
            + package_style_instruction
            + "The package must be structurally complete and fully visible: do not crop, truncate, erase, melt, overexpose, or hide its top, bottom, flaps, lid seam, handle, corners, lower edge, or other existing boundary. Keep the complete silhouette physically plausible. "
...
def _scene_instruction(category: str) -> str:
    ...
    if any(token in lowered for token in ("tool", "wrench", "hardware", "electric", "power")) or any(
        ...
        return (
            "Show the product in a real worksite or workshop as a natural casual phone snapshot; no hands, fingers, gloves, or human body parts. "
            "Use one continuous real camera photograph: no collage, split-screen, panels, contact sheet, comparison, inset, grid, divided frame, or separator lines. "
            "Place it according to its support area, center of gravity, cable direction, and contact points; no floating, impossible balance, staged upright pose, or unsupported vertical placement. "
            "Use practical lighting, slight handheld framing, believable scale, natural environment clutter, and no readable new text."
        )
    return (
        "Show a realistic product-in-use scene in a real category-appropriate environment, like a natural casual phone snapshot rather than a staged catalog render. "
        "Use one continuous real camera photograph: no collage, split-screen, panels, contact sheet, comparison, inset, grid, divided frame, or separator lines. "
        "The product must stand, lie, lean, or rest naturally according to its support area, center of gravity, cable direction, and contact points; no hands, fingers, gloves, human body parts, or unsupported placement. "
        "Without a visible stand, headphones or earphones must rest naturally on the desk with believable contact for headband, earcups, cable, controls, or microphone; never float, balance on one edge, or stand vertically. "
        "Use believable scale, practical lighting, slight handheld framing, natural placement details, and no readable new text."
    )
...
def _draw_qc_stamp(
    ...
    stamp_size: tuple[int, int] = (30, 30),
) -> None:
    draw = ImageDraw.Draw(card)
    x, y = xy
    red = (198, 45, 45, 220)
    ...
    diameter = min(stamp_width, stamp_height)
    draw.ellipse((x, y, x + diameter, y + diameter), outline=red, width=2)
    draw.line((x + 4, center_y, x + diameter - 4, center_y), fill=red, width=2)
    label_bbox = draw.textbbox((0, 0), "检验", font=stamp_font)
    label_width = label_bbox[2] - label_bbox[0]
    value_bbox = draw.textbbox((0, 0), value, font=value_font)
    value_width = value_bbox[2] - value_bbox[0]
    value_height = value_bbox[3] - value_bbox[1]
    draw.text((x + (diameter - label_width) / 2, y + 2), "检验", fill=red, font=stamp_font)
    draw.text((x + (diameter - value_width) / 2, center_y + (diameter // 2 - value_height) / 2 - 1), value, fill=red, font=value_font)
...
def _render_certificate_qc_stamp(inspector: str, font_path: str, scale: float) -> Image.Image:
    canvas_width = max(38, int(46 * scale))
    canvas_height = max(38, int(46 * scale))
    stamp = Image.new("RGBA", (canvas_width, canvas_height), (0, 0, 0, 0))
    offset = (max(4, int(7 * scale)), max(4, int(7 * scale)))
    stamp_dimension = max(20, int(30 * scale))
    _draw_qc_stamp(
        stamp,
        offset,
        inspector,
        _font(max(7, int(8 * scale)), font_path),
        _font(max(7, int(8 * scale)), font_path),
        (stamp_dimension, stamp_dimension),
    )
...
def _overlay_certificate_qc_stamp(image: Image.Image, project: dict[str, Any], font_path: str) -> Image.Image:
    ...
    for scale in (2 / 3, 0.6, 0.53, 0.47):
    ...
    if selected_stamp is None or selected_position is None:
        selected_stamp = _render_certificate_qc_stamp(inspector, font_path, 0.47)
